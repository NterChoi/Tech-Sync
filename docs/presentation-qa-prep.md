...
> **먼저 짚을 점: 이건 "서비스 분리"가 아니라 "저장소 분리"입니다.** 애플리케이션은 마이크로서비스가 아니라 **모놀리식(단일 Spring Boot)** 이고, 그 하나의 앱이 용도에 따라 여러 저장소에 붙는 구조(Polyglot Persistence)입니다. 서비스를 쪼갠 게 아니라 데이터 특성에 맞는 저장소를 골라 쓴 것뿐입니다.
...
> "애플리케이션 자체는 모놀리식 단일 Spring Boot이고, 서비스를 쪼갠 게 아니라
> 데이터 특성에 따라 저장소만 나눴습니다.
...
### Q11. "뉴스 피드의 소스/키워드 필터는 어떻게 구현했나요?"

**기존 방식의 한계:**

처음엔 피드 조회 쿼리가 "GEEK 전체 + 구독한 키워드의 NAVER 기사"를 **무조건 합쳐서** 반환하는
하나의 정적 쿼리(`@Query`)였습니다.

```java
@Query("{ '$or': [ { 'SOURCE':'GEEK' }, { 'SOURCE':'NAVER','KEYWORD':{'$in':?0} } ] }")
```

여기에 "GEEK만", "Naver만", "특정 키워드만" 같은 필터를 더하려면
조건 조합마다 `@Query` 메서드를 따로 만들어야 해서 메서드가 계속 늘어납니다.

**해결 — 동적 쿼리:**

`ArticleRepositoryCustom` 프래그먼트를 만들고 **MongoTemplate + Criteria**로
필터 조건을 런타임에 조립했습니다. 정렬·페이징은 `Pageable`에 그대로 위임합니다.

```java
// source/keyword 유무에 따라 Criteria를 동적으로 구성
if (source == GEEK)        → SOURCE = GEEK
else if (source == NAVER)  → SOURCE = NAVER AND KEYWORD in (구독목록)
else if (keyword != null)  → SOURCE = NAVER AND KEYWORD = keyword
else                       → GEEK 전체 OR (NAVER AND 구독 키워드)   // 기존 동작
```

API는 `/api/feed?source=GEEK|NAVER&keyword=...`로 받고, 프론트는 상단 칩(전체/GEEK/Naver + 구독 키워드)으로 노출합니다.

**이렇게 대답하세요:**

> "처음엔 GEEK 전체와 구독 키워드 NAVER를 합치는 정적 쿼리 하나였는데,
> 소스·키워드별 필터를 추가하려니 조건 조합마다 쿼리 메서드가 늘어나는 문제가 있었습니다.
> 그래서 MongoTemplate의 Criteria로 동적 쿼리를 조립하는 커스텀 리포지토리로 바꿔서,
> 한 메서드로 모든 필터 조합을 처리하고 정렬·페이징은 Pageable에 위임했습니다."

---

### Q12. "키워드 'Spring'으로 검색하면 계절 '봄' 기사가 섞이던데 어떻게 해결했나요?"

**문제:**

네이버 뉴스 API에 키워드 "Spring"을 그대로 검색하면 프레임워크가 아니라
**계절·패션 '스프링'** 기사가 섞여 들어옵니다. 검색 정확도(relevance) 문제는 완벽한 해결이 없습니다.

**해결 — 표시명과 검색어 분리:**

`KEYWORD_MASTER`에 `SEARCH_QUERY` 컬럼을 추가해서, 화면에 보이는 **표시명**과
실제 네이버에 던지는 **검색어**를 분리했습니다.

| 표시명(KEYWORD_NAME) | 검색어(SEARCH_QUERY) |
|---|---|
| Spring | Spring 프레임워크 |
| Vue | Vue.js |
| React | React.js |

검색은 보정된 질의어로 하되, **기사 저장·구독 매칭은 표시명 기준**이라 피드 로직은 그대로입니다.
코드에 하드코딩하지 않고 데이터(컬럼)로 관리하므로 키워드가 늘어도 코드 수정이 없습니다.

**이렇게 대답하세요:**

> "검색 정확도는 완벽히 해결할 수 없는 문제라, 키워드 마스터에 검색어 컬럼을 따로 둬서
> 표시명은 'Spring'으로 두되 실제 네이버 검색은 'Spring 프레임워크'로 던지도록 분리했습니다.
> 저장·구독 매칭은 표시명을 그대로 쓰기 때문에 피드 로직에는 영향이 없고,
> 코드가 아니라 데이터로 관리해서 키워드가 추가돼도 코드 변경이 필요 없습니다."

---

### Q13. "뉴스 제목에 `&quot;` 같은 글자가 보이던데 왜 그런가요?"

**원인:**

네이버 뉴스 API 응답은 제목/본문에 검색어 강조용 `<b>` 태그와 함께
**HTML 엔티티**(`&quot;` → `"`, `&amp;` → `&`)를 그대로 담아 보냅니다.
기존 코드는 정규식으로 `<태그>`만 제거하고 엔티티는 풀지 않아서 `&quot;`가 그대로 노출됐습니다.

**해결:**

Spring 내장 `HtmlUtils.htmlUnescape()`로 엔티티를 디코딩하는 한 단계를 추가했습니다(네이버·긱뉴스 수집기 양쪽).
이미 DB에 적재된 기존 기사도 일괄 정리(디코딩)했습니다.
...
> "네이버 API가 제목에 `<b>` 태그와 `&quot;` 같은 HTML 엔티티를 함께 주는데,
> 기존엔 태그만 지우고 엔티티는 안 풀어서 그대로 보였습니다.
> Spring의 HtmlUtils.htmlUnescape로 디코딩 단계를 추가했고,
> 이미 저장된 기사들도 한 번에 정리했습니다."

---

### Q14. "마이페이지의 비밀번호 변경과 회원 탈퇴는 어떻게 처리했나요?"

**비밀번호 변경:**

1. **현재 비밀번호**를 `passwordEncoder.matches()`로 검증 (BCrypt 비교)
2. 통과하면 새 비밀번호를 인코딩해 저장
3. **Redis의 Refresh Token을 삭제** → 기존 세션 무효화(재로그인 강제)

비밀번호를 바꿨는데 예전 토큰으로 계속 접근되면 안 되므로, 변경 즉시 세션을 끊습니다.
(아이디/비번 찾기의 재설정도 같은 이유로 토큰을 무효화합니다.)

**회원 탈퇴:**

한 트랜잭션에서 연관 데이터를 정리한 뒤 계정을 삭제합니다.

```
구독 키워드 삭제 → 스크랩 삭제 → 워크스페이스 멤버십 삭제
→ Redis Refresh Token 삭제 → USER_INFO 삭제
```

단, **본인이 만든 워크스페이스(OWNER) 자체는 남깁니다.** 공유 워크스페이스를 탈퇴로 통째로
지우면 다른 멤버의 데이터까지 사라지기 때문에, 멤버십만 정리하는 선택을 했습니다.

**이렇게 대답하세요:**

> "비밀번호 변경은 현재 비밀번호를 BCrypt로 먼저 검증하고, 통과하면 새 비밀번호를 저장한 뒤
> Redis의 Refresh Token을 삭제해 기존 세션을 무효화합니다.
> 회원 탈퇴는 한 트랜잭션에서 구독 키워드·스크랩·워크스페이스 멤버십·토큰을 정리하고 계정을 삭제하는데,
> 본인이 만든 공유 워크스페이스는 다른 멤버 영향 때문에 남기고 멤버십만 제거하도록 했습니다."

---

## 여기까지 읽었으면 충분합니다

위 14개 질문은 "JWT + Spring Security + 실시간 협업 + 뉴스 피드"를 발표할 때
